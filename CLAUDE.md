...
- After making code changes to TypeScript/JavaScript files, ALWAYS run ESLint on the affected workspace (e.g., `pnpm --filter @sound-connect/web lint` for web changes, `pnpm --filter @sound-connect/api lint` for API changes). If there are linting errors, fix them immediately before proceeding.

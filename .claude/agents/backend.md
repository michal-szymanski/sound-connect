...
- [ ] Has schema been designed (if database changes needed)?
  - If no: Consult database-architect agent before making schema changes
- [ ] Are shared Zod schemas available in `packages/common`?
  - If no: Coordinate with system-architect to create them

**For schema changes:**
- [ ] Has database-architect reviewed the design?
...
AskUserQuestion({
  questions: [{
    question: "This feature requires database changes, but I don't see a schema design. Should I consult database-architect first?",
    header: "Pre-flight",
    options: [
      { label: "Yes, consult database-architect", description: "Ensure optimal schema design" },
      { label: "No, proceed with my design", description: "Faster, but may not be optimal" }
    ],
    multiSelect: false
  }]
})
